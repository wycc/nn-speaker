...
print(f"Image shape: {IMG_WIDTH}x{IMG_HEIGHT}")

# Binary classification: wow=1, everything else=0
Y_train = [1 if y == words.index('wow') else 0 for y in Y_train_cats]
Y_validate = [1 if y == words.index('wow') else 0 for y in Y_validate_cats]
Y_test = [1 if y == words.index('wow') else 0 for y in Y_test_cats]

batch_size = 30

train_dataset = Dataset.from_tensor_slices(
    (X_train, Y_train)
).repeat(count=-1).shuffle(len(X_train)).batch(batch_size)
...
model = Sequential([
    ...
    Flatten(),
    Dropout(0.2),
    Dense(40, activation='relu', kernel_regularizer=regularizers.l2(0.001), name='hidden_layer1'),
    Dense(1, activation='sigmoid', kernel_regularizer=regularizers.l2(0.001), name='output')
])
model.summary()
...
print(tf.math.confusion_matrix(Y_test, decision).numpy())

complete_train_X = np.concatenate((X_train, X_validate, X_test))
complete_train_Y = np.concatenate((Y_train, Y_validate, Y_test))

complete_train_dataset = Dataset.from_tensor_slices(
    (complete_train_X, complete_train_Y)
).repeat(count=-1).shuffle(300000).batch(batch_size)
...
print("Model saved as fully_trained.model")
